...
- Token: `iss = LIVEKIT_API_KEY`, `sub = user.id`, `exp` = `expires_at` của chỗ, với chỗ = max(chỗ đang giữ, `now + 120s`), `video = { room: roomId, roomJoin: true, canPublish: true, canSubscribe: true }`. **Không** `roomCreate`/`roomAdmin`/`roomList`. Tên phòng phía LiveKit **là** `rooms.id`.
...
- 2026-09-13 (QUYẾT ĐỊNH CON NGƯỜI, sửa khối đóng băng theo thương lượng lại sau review vòng 2): chấp nhận câu mới cho token trong `## Boundaries & Constraints`: "`exp` = `expires_at` của chỗ; chỗ = max(chỗ đang giữ, `now + 120s`)" thay cho "`exp = now + 120s`". Mã không đổi (`GREATEST` ở adapter PG, `Math.max` ở in-memory, đã có từ patch 6 vòng 1); ca ghim `never moves the expiry BACKWARDS` giữ nguyên. Mục độ lệch ngay trên đóng lại bằng quyết định này.
