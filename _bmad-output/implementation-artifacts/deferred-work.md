...
- source_spec: `_bmad-output/implementation-artifacts/spec-1-2-dang-nhap-bang-bon-provider.md`
  ...
  evidence: Không có endpoint GET /v1/auth/providers và không có schema cho nó trong packages/contracts. **Không còn chờ Story 1.6**: 1.6 đã dựng lại trang (hệ thiết kế "Cắm trại"; `sign-in-links.tsx` giờ render bốn `button-secondary`) và cố ý KHÔNG đụng tới danh sách provider — spec 1.6 ghi rõ đó là việc của backend. Mục này vẫn mở và giờ vô chủ: cần một endpoint ở `apps/api` cộng một schema ở `packages/contracts` trước khi cắm credential thật, vì tới lúc đó provider chưa bật vẫn cho ra một nút dẫn thẳng tới 404.
...
- source_spec: `_bmad-output/implementation-artifacts/spec-1-3b-rate-limit-dang-nhap.md`
  ...
  evidence: **ĐÃ ĐÓNG ở Story 1.6**, và không phải bằng jsdom. Đã thu hẹp qua ba vòng: vòng 1 chỉ grep source tìm chữ setTimeout; vòng 2 test hàm thuần nextTickDelayMs; vòng 3 tiêm `clock` thành prop nên renderToStaticMarkup render được component thật ở hai thời điểm và so số — phần dư còn lại là việc React có thật sự gọi callback trong setTimeout hay không, thứ mà làm rỗng callback vẫn để mọi test xanh. 1.6 cần một trình duyệt thật cho ba trạng thái theme, nên nó có sẵn chỗ đóng luôn phần dư: `tests/e2e/web/he-thiet-ke.spec.ts` mở `/dang-nhap?ket-qua=bi-khoa&giay=30` trong Chromium dưới `reducedMotion: 'reduce'`, đọc `.notice-countdown` hai lần và khẳng định con số ĐÃ GIẢM. Làm rỗng callback thì ca đó đỏ. Ca này phục vụ hai mục cùng lúc — nó cũng là ca "reduced-motion không chạm đồng hồ" của 1.6, vì đồng hồ chạy bằng `setTimeout` chứ không bằng CSS. Repo vẫn KHÔNG có jsdom.
...
- source_spec: `_bmad-output/implementation-artifacts/spec-1-3b-rate-limit-dang-nhap.md`
  ...
  status: ĐÃ ĐÓNG (2026-09-05). `apps/api/src/__testing__/api-env.ts` nạp chuỗi thô qua `parseApiEnv`, đúng hàm `loadApiConfig` gọi lúc khởi động — một biến bắt buộc mới làm mọi caller ném lỗi có tên biến. Ghi chú ngược lại mục này: builder "có kiểu đầy đủ" như đề xuất ban đầu sẽ KHÔNG đóng được lỗ, vì `apps/api/tsconfig.json` loại trừ `src/**/*.test.ts` nên bốn file đó không được kiểu kiểm bởi bất cứ thứ gì. Luật quét được đi kèm: `tests/gates/config-cast-ban.test.ts`.
...
- source_spec: `_bmad-output/implementation-artifacts/spec-1-3c-phien-het-han-quay-lai.md`
  ...
  evidence: Dialog mở ở `/phong-hoc/a`, người dùng điều hướng client-side sang `/phong-hoc/b` rồi mới bấm "Tiếp tục với Google" thì vẫn bị đưa về `/phong-hoc/a`. Test `follows the person to a new location between two 401s` chỉ phủ trường hợp có 401 THỨ HAI cập nhật lại prompt. Không vá trong vòng này vì mọi cách sửa đều đẩy một lần đọc `window.location` vào chỗ không test được: href được dựng lúc render, nên muốn đúng lúc bấm thì phải có `onClick` đọc `window.location` rồi ghi đè `href` — đúng loại code mà `vitest.config.mts:119` (project `web`, `environment: node`, không DOM) không chạy được, và spec 1.3c bắt mọi quyết định nằm trong hàm thuần. Hệ quả thực tế nhỏ: người dùng về nhầm một trang trong cùng ứng dụng, không phải mất phiên. Sửa đúng cách cần một chỗ chạy được `onClick` đọc `window.location`, hoặc một `usePathname()` trong provider kèm Suspense boundary. **Story 1.6 KHÔNG đóng mục này và cũng không còn là chỗ dựa của nó**: 1.6 cố ý không thêm jsdom (spec cấm), nên project `web` vẫn không có DOM. Cái 1.6 để lại là một đường mới: `tests/e2e/web/` giờ chạy Chromium thật, nên một ca Playwright điều hướng client-side giữa hai trang rồi mới bấm nút trong dialog LÀ kiểm được — thứ trước đây không có chỗ nào chạy. Ai nhận mục này nên viết ca đó trước, rồi mới sửa.

- source_spec: `_bmad-output/implementation-artifacts/spec-1-3c-phien-het-han-quay-lai.md`
  summary: Dialog phiên hết hạn xuất hiện mà không có gì thông báo cho bàn phím và trình đọc màn hình.
  evidence: **ĐÃ ĐÓNG ở Story 1.6.** Trạng thái cũ: không chuyển focus, không `aria-live`, không `role="alertdialog"` — người dùng bàn phím đang ở cuối trang không biết vừa có bốn liên kết đăng nhập xuất hiện, và trình đọc màn hình im lặng vì phần tử được THÊM vào cây chứ không phải nội dung đổi trong một live region đã tồn tại. Cách đóng: `session-expiry-dialog.tsx` nhận `tabIndex={-1}` cộng một `focusRef` bắt buộc, `session-expiry-provider.tsx` có `useEffect` chuyển focus vào dialog khi `prompt` khác `null`, và Escape đóng dialog bằng `onKeyDown` ngay trên phần tử chứ không phải listener trên `document`. Hai ca Playwright trong `tests/e2e/web/dang-nhap.spec.ts` chứng minh: focus thật sự vào dialog và Escape đóng nó; và trang phía sau KHÔNG bị khoá — `position` không phải `fixed`, `overflow` của `html`/`body` không phải `hidden`, `body` không mang `inert`, và năm lần Tab đi từ dialog qua bốn liên kết tới nút "Để sau" thay vì quay vòng.
  Hai đề nghị trong mục cũ bị TỪ CHỐI có lý do chứ không phải bị bỏ quên. "Một live region luôn mount ở layout" cộng với chuyển focus sẽ đọc dialog hai lần. Và "focus-trap" mâu thuẫn trực tiếp với `session-expiry-dialog.tsx:13-26`, chỗ lập luận rằng nhốt màn hình chính là cái hại Story 1.3c tồn tại để chặn — nhốt focus là modal đội tên khác. Docblock thắng.
...
- source_spec: `_bmad-output/implementation-artifacts/spec-1-4-khai-ngay-sinh.md`
  ...
  status: ĐÃ ĐÓNG (2026-09-05). `tests/e2e/web/` chạy bundle Next.js thật trong Chromium với một origin giả (`tests/e2e/support/fake-api.cjs`) — 10 ca cho `/khai-ngay-sinh`, `/dang-nhap` và dialog hết phiên của 1.3c, đúng như mục này đề nghị làm một lần cho cả ba. CI có job riêng `E2E · các màn hình web trong trình duyệt`. GIỚI HẠN, ghi rõ để vòng sau không đọc nhầm: nó chứng minh phía CLIENT, không chứng minh `apps/api` đồng ý — thân phản hồi được validate bằng schema thật của `@stuwith/contracts`, còn MÃ TRẠNG THÁI thì không, và chính chỗ đó đã lộ ra lỗi 204/200 của seam 1.3c.
...
- source_spec: `_bmad-output/implementation-artifacts/spec-1-4-khai-ngay-sinh.md`
  ...
  status: ĐÃ ĐÓNG (2026-09-05). `playwright.config.ts` giờ có bốn `webServer` (thêm origin giả và `apps/web`) và hai project (`api` không trình duyệt, `web` chạy Chromium). `apps/web` được BUILD trong chính webServer đó với `NEXT_DIST_DIR=.next-e2e`, vì `NEXT_PUBLIC_API_BASE_URL` được nội tuyến lúc build — khởi động lại một bundle cũ sẽ gọi origin khác và cho một lượt chạy xanh vô nghĩa.
...
- source_spec: `_bmad-output/implementation-artifacts/spec-1-3c-phien-het-han-quay-lai.md`
  summary: `SESSION_EXPIRED_STATUS` và `RATE_LIMITED_STATUS` đã chuyển sang `packages/contracts`, nhưng chưa có luật nào ngăn một hằng biên `/v1` mới được khai lại bên trong một process.
  evidence: Lỗi 204/200 của seam gia hạn (tìm thấy 2026-09-05 bởi lượt chạy e2e đầu tiên) có gốc là ba hằng số trạng thái nằm trong `apps/web/src/app/session-expiry.ts`. Đã sửa bằng cách chuyển cả ba vào `packages/contracts` và cho hai process cùng import, nên lần này không thể lệch nữa. Nhưng đó là sửa CA, không phải sửa LỚP: hằng biên tiếp theo — một mã trạng thái, một tên header — vẫn có thể được khai trong `apps/web` hoặc `apps/api` mà không gì bắt. Luật đúng là quét mã trạng thái đứng dạng literal trong nhánh điều kiện của hai app, kiểu `tests/gates/config-cast-ban.test.ts`. Không làm ngay vì cần định nghĩa cẩn thận "literal nào là hằng biên" để không báo động giả trên mọi `=== 200`.

- source_spec: `_bmad-output/implementation-artifacts/spec-1-4-khai-ngay-sinh.md`
  summary: E2E chưa chạy `apps/api` thật — origin giả không chứng minh được hai bên đồng ý về MÃ TRẠNG THÁI.
  evidence: `tests/e2e/support/fake-api.cjs` validate mọi THÂN phản hồi bằng schema thật của `@stuwith/contracts`, nên thân không thể trôi. Mã trạng thái thì không có gì ràng: bản giả trả 204 cho lượt khai ngày sinh trong khi API thật trả 200, và chỉ lộ ra khi spec đỏ. Muốn đóng thật cần e2e chạy `apps/api/dist/main.js` với Postgres + Valkey qua Testcontainers và một authorization server giả — tức phải cho discovery URL của provider cấu hình được, một thay đổi lộ ra production để phục vụ test. Cân nhắc lại ở Epic 2 khi phòng học đã cần một stack e2e đầy đủ.

- source_spec: `_bmad-output/implementation-artifacts/spec-1-5-cong-chan-hanh-vi-co-tien.md`
  summary: Story 1.5 KHÔNG đóng mục "không có cổng chặn 'chưa khai ngày sinh thì không đi đâu khác'" ở phía `apps/web` — và đó là quyết định, không phải bỏ sót.
  evidence: Mục 1.4 ở trên viết "Story 1.5 dựng cổng chặn hành vi có tiền trên cùng cờ này, nên đó là chỗ làm chặt một lần". Không làm được: danh sách Never của spec 1.5 nói thẳng "Không đụng `apps/web`" — ẩn nút thuộc Story 3.3 (và AC của 3.3 nói nút *không tồn tại* chứ không phải bị làm mờ), còn cổng hồ sơ chưa hoàn tất là nợ của 1.4. Hai luật cũng không cùng một luật: cổng tiền hỏi `canReceiveMoney` (đủ 18), cổng hồ sơ hỏi `isProfileComplete` (đã khai). Chúng trùng nhau ở trạng thái `not-declared` và tách nhau ở mọi trạng thái khác, nên gộp chúng vào một middleware sẽ chặn người 17 tuổi ra khỏi những trang họ được phép xem. Chủ sở hữu hôm nay: vẫn không ai.

- source_spec: `_bmad-output/implementation-artifacts/spec-1-5-cong-chan-hanh-vi-co-tien.md`
  summary: `isOver18` trong `packages/contracts` vẫn không có consumer sản phẩm nào sau Story 1.5, và điều đó giờ là một quyết định đã chốt chứ không còn là câu hỏi mở.
  evidence: Mục 1.4 ở trên ghi "consumer thật là cổng chặn hành vi có tiền của Story 1.5 (`canReceiveMoney`)". Hoá ra không phải: `canReceiveMoney` sống ở `packages/domain` và `MoneyGateGuard` gọi nó với `User` đọc thẳng từ `IdentityPort` — cờ `is_over_18` trên dây KHÔNG bao giờ là cơ sở của một quyết định bảo mật, vì nó là thứ client cầm và client không được phép quyết định. `isOver18` do đó chỉ còn nghĩa cho giao diện (Story 3.3 ẩn nút). Giữ lại có chủ đích; nếu Story 3.3 vẫn không gọi nó thì đó là lúc xoá.

- source_spec: `_bmad-output/implementation-artifacts/spec-1-5-cong-chan-hanh-vi-co-tien.md`
  summary: Một lượt bị cổng tiền từ chối không sinh dòng audit nào — không ai đếm được, không ai cảnh báo được.
  evidence: `MoneyGateGuard` ném exception và không chạm `AuditPort`. Cố ý trong phạm vi story này: `audit_events.action` là CHECK constraint chép tay vào migration, thêm một giá trị là thay đổi ba chỗ và thuộc mục Ask First; và Story 1.7 mới là chủ của đường audit thật. Nhưng hệ quả là thật: luồng report/moderation cho hành vi khai gian tuổi (Epic 4) sẽ cần biết "tài khoản này đã đâm vào cổng tiền bao nhiêu lần", và hôm nay không có gì trả lời được. Ai làm: Story 1.7 khi khai `action` mới, hoặc Epic 4 khi dựng luồng report.

- source_spec: `_bmad-output/implementation-artifacts/spec-1-5-cong-chan-hanh-vi-co-tien.md`
  summary: Endpoint đánh dấu `@MoneyIn()` KHÔNG tự động có rate limit, và không có cách nào viết `@RateLimited(...)` lên nó hôm nay.
  evidence: `RateLimitAction` trong `packages/domain` chỉ có tên cho bốn route `/v1/auth`; không có giá trị nào viết được trong ngoặc cho một route tiền. Nên một endpoint tiền mới đi qua `RateLimitGuard` như một no-op. Với `/v1/auth` đó là đúng (xem `POST /v1/auth/logout`), với một endpoint nhận coin thì không: một vòng lặp gọi vào endpoint tiền là thứ cần đếm. Epic 3 phải quyết định — thêm `RateLimitAction` cho chiều tiền, hoặc một dimension riêng theo user — TRƯỚC khi endpoint tiền thật đầu tiên lên production. Ghi ở đây vì hôm nay nó vô hại (chỉ có fixture trong test) và ngày mai thì không.

- source_spec: `_bmad-output/implementation-artifacts/spec-1-5-cong-chan-hanh-vi-co-tien.md`
  summary: `pino-http` serialize RAW Node request, không phải Fastify request — nên serializer `req` hôm nay không nhìn thấy thứ guard gắn lên, và Story 1.7 phải biết điều đó TRƯỚC khi đổi đối tượng được serialize.
  evidence: ĐÃ ĐÓNG phần kiểm chứng trong story này. `apps/api/src/logging.test.ts` có suite "a request through the money gate leaks nothing into a real pino (AD-15)": chạy `apps/api` thật, đăng nhập hai người có ngày sinh thật, gọi route `@MoneyIn()` ở cả ba đường (200, 403, 500 do handler ném), rồi đọc lại mọi dòng pino đã ghi. Suite đã được kiểm bằng mutation: cho handler ném lỗi kèm ngày sinh thì hai ca đỏ ngay. Cái CÒN LẠI cho 1.7 là một sự thật đã đo chứ không phải một lo ngại: `pino-http` serialize `IncomingMessage` thô, còn `MoneyGateGuard` gắn `AuthenticatedCaller` lên FASTIFY request bọc ngoài — thử viết lại serializer `req` để duyệt `Object.getOwnPropertySymbols(req)` và đọc `.user.dateOfBirth` thì KHÔNG lộ gì, vì nó đang nhìn một đối tượng khác. Nghĩa là hàng rào hôm nay không phải khoá symbol mà là "pino không thấy đối tượng đó". Nếu whitelist serializer của 1.7 chuyển sang serialize Fastify request (một lựa chọn hợp lý, vì đó mới là nơi có `routerPath`, `params`, `id` của Fastify) thì hàng rào đó biến mất và chỉ còn khoá symbol đứng — lúc đó `Reflect.ownKeys` hay một deep-clone nào đó là đủ để đẩy ngày sinh vào một dòng log không xoá được. Chạy lại suite trên sau khi đổi, đừng chỉ chạy suite đăng nhập.

- source_spec: `_bmad-output/implementation-artifacts/spec-1-5-cong-chan-hanh-vi-co-tien.md`
  summary: `@MoneyIn()` chỉ đúng khi NGƯỜI GỌI là người NHẬN. Epic 3 phải quyết định cách chặn một endpoint mà người gửi và người nhận là hai người khác nhau.
  evidence: Guard hỏi `canReceiveMoney(caller.user)` — tức là hỏi về người đang gọi. Ba hành vi epic nêu (bật nhận hỏi riêng, đặt đơn giá, nhận coin) đều có người gọi chính là người nhận, nên story này đúng. Nhưng một endpoint kiểu "người gửi POST một khoản tip" mà mang cùng cái mark sẽ sai theo CẢ HAI chiều cùng lúc: nó từ chối người TRẢ 17 tuổi (điều spec nói rõ là được phép), và nó cho một người lớn trả tiền cho một người dưới 18 (đúng thứ luật này tồn tại để chặn). Không lỗi nào trông giống lỗi: endpoint có mark, guard có chạy, test xanh. Docblock của `money-in.decorator.ts` viết rõ ràng buộc này; cái CHƯA có là một cơ chế. Cần một `@MoneyInFor(recipient)` hoặc một cổng ở tầng service đọc người nhận từ payload — quyết định thuộc Epic 3, khi đã có một endpoint thật để đo.

- source_spec: `_bmad-output/implementation-artifacts/spec-1-5-cong-chan-hanh-vi-co-tien.md`
  summary: Không gì buộc một handler đã đánh dấu phải DÙNG người gọi mà guard đã giải.
  evidence: `moneyInCallerOf(request)` là một lời mời, không phải một ràng buộc. Một handler Epic 3 hoàn toàn có thể ghi có cho một `user_id` đọc từ thân request và bỏ qua người gọi — guard vẫn cho qua (vì người GỌI đủ 18), còn tiền thì đi tới một người khác mà không ai hỏi tuổi. Đây là cùng một lớp lỗi với mục trên nhưng ở phía handler chứ không phía decorator. Cách đóng thật là kiểu dữ liệu: handler nhận người nhận đã được cổng phê duyệt như một tham số, thay vì tự đọc từ `body`. Cần một endpoint thật mới thiết kế được, nên thuộc Epic 3.

- source_spec: `_bmad-output/implementation-artifacts/spec-1-5-cong-chan-hanh-vi-co-tien.md`
  summary: Guard gắn NGUYÊN `User` lên request, trong khi handler thật gần như chắc chắn chỉ cần `id`.
  evidence: `AuthenticatedCaller` mang cả `dateOfBirth` và `email` — hai trường PII — chỉ vì `SessionAuthenticator` trả về `User`. Thu hẹp xuống đúng những trường một handler cần sẽ XOÁ bề mặt rò rỉ thay vì phải viết tài liệu và test cho nó: `money-gate.request.test.ts`, nửa suite PII trong `logging.test.ts` và mục deferred về serializer của 1.7 đều tồn tại vì object đó rộng hơn mức cần. Chưa làm vì hôm nay chưa có handler thật nào để biết "những trường cần" là gì, và đoán sai sẽ phải mở lại. Xem lại khi Epic 3 có endpoint tiền đầu tiên.

- source_spec: `_bmad-output/implementation-artifacts/spec-1-5-cong-chan-hanh-vi-co-tien.md`
  summary: `MONEY_IN_FORBIDDEN_MESSAGE` chưa tới được màn hình nào — một lượt bị cổng tiền từ chối hiện ra là không có gì.
  evidence: Câu chữ đã nằm ở `packages/contracts` (AD-13) và envelope 403 đã đúng, nhưng `apps/web` cố ý cho 403 đi thẳng qua: `authorizedCall` chỉ rẽ nhánh trên 401, và `session-expiry.test.ts` giờ có ca khẳng định 403 KHÔNG gia hạn phiên và KHÔNG bật dialog — đó là hành vi đúng. Hệ quả là hôm nay chưa có chỗ nào đọc `error.message` của một 403 và hiển thị nó. Story 3.3 sở hữu việc này, và AC của 3.3 nói nút *không tồn tại* với người chưa đủ 18 — nên câu này là mạng lưới an toàn cho đường gọi thẳng API, không phải văn bản chính của màn hình. Ghi lại để 3.3 hoặc dùng nó, hoặc nói rõ vì sao không.

- source_spec: `_bmad-output/implementation-artifacts/spec-1-5-cong-chan-hanh-vi-co-tien.md`
  summary: `stripComments` giờ tồn tại ở hai project test, và tính đúng đắn của nó phụ thuộc vào một chi tiết neo mà cả hai bản đều phải comment lại.
  evidence: `tests/gates/config-cast-ban.test.ts` và `apps/api/src/money/money-gate.flow.test.ts` mỗi bên giữ một bản. Cả hai đều phải neo luật comment dòng vào đầu dòng (`/^[ \t]*\/\/.*$/gm`): bản KHÔNG neo sẽ ăn hết phần sau `//` trong một URL, khiến dòng chứa vi phạm biến mất — chính là lỗi `config-cast-ban.test.ts` đã ship một lần và ghi lại trong docblock. Hai bản sao của một luật là hai bản sao sẽ trôi. Không gộp ngay vì chúng ở hai vitest project khác nhau (`gates` và `api`) và chưa có package test-util dùng chung; chỗ đúng có lẽ là `packages/db`'s test-kit hoặc một package mới, và đó là quyết định về hạ tầng test chứ không phải về story này.

- source_spec: `_bmad-output/implementation-artifacts/spec-1-6-he-thiet-ke-cam-trai.md`
  summary: `DESIGN.md` TỰ MÂU THUẪN về `border-decor`, và Story 1.6 phải chọn một bên để ship.
  evidence: `:274` và `:386` nói `border-decor` "chỉ dùng cho vạch phân cách trang trí, không bao giờ là ranh giới duy nhất của một thành phần bấm được". `:362` (`chat-panel.composer-border`) lại kê `border-decor` nét đứt cho "mọi ô nhập rỗng". Ô `type="date"` ở `/khai-ngay-sinh` là thành phần bấm được và không có ranh giới nào khác, nên hai câu không thể cùng đúng. Số đo đứng về phía `:274`: `border-decor` trên `surface-raised` là **1.41:1** ở light và **1.33:1** ở dark, trong khi WCAG 1.4.11 đòi ≥ 3:1 cho ranh giới một thành phần giao diện — tức là câu `:362` một mình sẽ ship một ô nhập mà người thị lực kém không thấy mép. 1.6 hoà giải trong CODE bằng cách giữ KIỂU nét đứt (thứ `:362` mua — tín hiệu "chỗ để điền") và đổi MÀU sang `border-ink` (16.41:1 / 3.55:1), có luật quét allow-list trong `design-tokens.test.ts` giữ `.field` khỏi quay lại. **Không sửa `DESIGN.md`** — spec 1.6 xếp việc đổi giá trị trong đó vào mục "Ask First". Câu nào là chuẩn là quyết định của con người, và quyết định đó ảnh hưởng tới `chat-panel` của Epic 2 chứ không chỉ ô nhập này.

- source_spec: `_bmad-output/implementation-artifacts/spec-1-6-he-thiet-ke-cam-trai.md`
  summary: Focus được chuyển VÀO dialog phiên hết hạn và không bao giờ được trả lại.
  evidence: `session-expiry-provider.tsx` gọi `dialogRef.current?.focus()` ở lượt chuyển đóng→mở, và không giữ lại nơi người dùng đang đứng. Sau khi bấm Escape hoặc "Để sau", focus rơi về `<body>`, nên lần Tab tiếp theo bắt đầu lại từ đầu trang — ai đang khai ngày sinh dở sẽ mất chỗ và phải Tab qua header, nút đổi giao diện rồi mới về tới ô nhập. Đây là lựa chọn CÓ Ý THỨC chứ không phải bỏ sót: dialog mở ra vì một request nền gặp 401, nên phần tử đang focus lúc đó hoàn toàn có thể đã unmount trước khi người dùng đóng dialog, và `element.focus()` trên một node đã rời khỏi cây là một lỗi im lặng khác. Cách đóng đúng cần một chiến lược khôi phục có kiểm tra `isConnected` cộng một fallback, và cần một màn hình dài thật để đo — Epic 2 có màn hình đó. Ghi lại vì hôm nay không có gì nói ra điều này.

- source_spec: `_bmad-output/implementation-artifacts/spec-1-6-he-thiet-ke-cam-trai.md`
  summary: Không có đồng bộ giao diện giữa các tab.
  evidence: `theme-switch.tsx` ghi `localStorage` và đổi thuộc tính trên `document.documentElement` của CHÍNH tab đó. Mở hai tab StuWith, đổi sang "Tối" ở tab một: tab hai vẫn sáng, nút "Theo hệ điều hành" của nó vẫn `aria-pressed="true"`, và live region của nó vẫn nói "Đang dùng giao diện sáng" — cho tới khi tải lại. Không phải lỗi dữ liệu (lần tải sau đọc `localStorage` và ra đúng), nhưng là hai tab của một sản phẩm nói hai điều khác nhau về cùng một lựa chọn. Cách đóng đã biết và nhỏ: một listener `window.addEventListener('storage', …)` lọc theo `THEME_STORAGE_KEY`, chạy lại `resolveThemeChoice` rồi áp thuộc tính. Chưa làm trong 1.6 vì `storage` event không phát cho tab đã ghi, nên nó cần ca Playwright hai `page` trong một context — một khuôn mà bộ e2e hiện tại chưa có, và dựng khuôn đó là việc lớn hơn chính bản vá.

- source_spec: `_bmad-output/implementation-artifacts/spec-1-6-he-thiet-ke-cam-trai.md`
  summary: i18n (VI + EN) không thuộc story nào, và ba docblock từng nói dối rằng nó thuộc Story 1.6.
  evidence: `EXPERIENCE.md:27` viết "Ngôn ngữ: **VI mặc định, EN song song**. Mọi chuỗi đều phải qua i18n ngay từ đầu — không có chuỗi cứng, kể cả trong thông báo lỗi." Nhưng `epics.md` giao cho Story 1.6 hệ thiết kế và không có một chữ nào về i18n, trong khi `layout.tsx:30`, `sign-in-links.tsx:25` và `dang-nhap/sign-in-outcome.tsx:52` đều ghi "EN arrives with Story 1.6" / "full i18n is Story 1.6". Ba docblock hứa một việc không story nào nhận là cách "chắc ai đó đã làm rồi" hình thành ba story sau. 1.6 đã sửa cả ba câu và ghi việc thật ở đây. Hiện trạng: mọi chuỗi hiển thị là literal tiếng Việt nằm ngay trong component; không có thư viện i18n, không có file thông điệp, không có `lang` nào ngoài `<html lang="vi">` cố định trong `layout.tsx`. Chi phí thật không nằm ở việc dịch mà ở việc RÚT chuỗi ra khỏi khoảng tám file rồi làm `<html lang>` thành động — và `EXPERIENCE.md § Accessibility Floor` còn buộc mỗi đoạn tiếng Anh nhúng trong câu tiếng Việt phải mang `lang="en"`. Cần một con người quyết định phase nào nhận; càng để lâu càng nhiều chuỗi phải rút.

- source_spec: `_bmad-output/implementation-artifacts/spec-1-6-he-thiet-ke-cam-trai.md`
  summary: `next build` của `apps/web` giờ CẦN MẠNG ở lần build đầu trên một máy, và không có gì nói ra điều đó khi nó hỏng.
  evidence: `layout.tsx` dùng `next/font/google` với `Be_Vietnam_Pro` — không phải dependency mới (nó nằm trong `next`), và nó tự host font lúc build nên trình duyệt người dùng không hề gọi Google lúc chạy. Giá phải trả nằm ở phía build: lần đầu trên một máy chưa có cache, Next tải woff2 từ `fonts.gstatic.com`. Trên một runner CI không ra được internet, `next build` sẽ hỏng ở một bước không liên quan gì tới code vừa đổi, với thông báo không nhắc gì tới font. Đã ĐO ngày 2026-09-06 trên máy này: mạng có, `pnpm --filter web build` exit 0. Phương án hai đã được spec 1.6 nêu và nó là quyết định của CON NGƯỜI: commit các file woff2 vào repo và tự host bằng `next/font/local`. Chưa làm vì hôm nay build được, và vì nó thêm vài trăm KB nhị phân vào repo — một đánh đổi cần người chốt chứ không phải một lần sửa. Ghi ở đây để lần đầu CI đỏ vì lý do này, không ai phải đi tìm.

- source_spec: `_bmad-output/implementation-artifacts/spec-1-6-he-thiet-ke-cam-trai.md`
  summary: Hệ thiết kế có ba component base; `DESIGN.md` đặc tả mười sáu.
  evidence: `globals.css` dựng `button-primary`, `button-secondary`, `chip-status` — đúng ba khoá spec 1.6 yêu cầu, vì đó là ba thứ ba màn hình hiện có dùng tới. Mười ba khoá còn lại trong `DESIGN.md § components` (`tile-participant`, `control-bar`, `face-mode-panel`, `badge-credential`, `card-ask-confirm`, `card-coin`, `transaction-row`, `countdown-display`, `progress-coin`, `chat-panel`, `dialog-busy`, `strip-join-request`, `snackbar`) chưa có class nào, và đó là lựa chọn đúng chứ không phải thiếu sót: dựng sẵn một component không màn hình nào render vi phạm chính `routes.test.ts` rule C của repo này (ký hiệu chỉ test dùng là ký hiệu chết), và nó sẽ phải sửa lại khi màn hình thật xuất hiện. Tầng token thì đã đủ — `tokens.css` mang trọn palette, thang chữ, bo góc, cách và motion — nên Epic 2/3 chỉ cần thêm class chứ không cần thêm giá trị nào. Ghi lại để không ai tưởng hệ thiết kế đã "xong" theo nghĩa mười sáu component.

- source_spec: `_bmad-output/implementation-artifacts/spec-1-6-he-thiet-ke-cam-trai.md`
  summary: `chip-status` có class và không có chỗ dùng — nó tồn tại vì là khoá chung, không vì màn hình nào cần.
  evidence: Spec 1.6 bắt ba khoá component `button-primary` / `button-secondary` / `chip-status` phải có mặt dưới đúng tên đó, và `design-tokens.test.ts` kiểm điều đó. Nhưng ba màn hình của Epic 1 không có trạng thái mạng, không có coin và không có ô người tham gia, nên không màn hình nào render một `chip-status`. Nó là class CSS chứ không phải ký hiệu TypeScript nên `routes.test.ts` rule C không chạm tới, tức là hôm nay KHÔNG có gì chứng minh nó đúng ngoài chính luật quét tên. Ba biến thể `ok` / `warn` / `coin` cũng chưa từng được một trình duyệt nào render. Epic 2 là chỗ nó thật sự xuất hiện; ai làm story đó nên nhìn nó bằng mắt ở cả hai chế độ trước khi tin, và nên cân nhắc một luật quét cho class CSS chết tương đương rule C.

- source_spec: `_bmad-output/implementation-artifacts/spec-1-5-cong-chan-hanh-vi-co-tien.md`
  summary: `apps/realtime-gateway` không có cổng tiền tương đương, và không tài liệu nào nói nó nằm ngoài phạm vi.
  evidence: Spec 1.5 nói "chặn ở tầng API" và mọi thứ đã dựng nằm trong `apps/api`. Nhưng `apps/realtime-gateway` là process thứ hai, có ranh giới WebSocket xác thực bằng chính session của `api`, và epic context giao cho nó "coin scheduler, phiên, chat, presence" — tức là đường tiền của Epic 3 sẽ đi qua nó. Hôm nay nó không có `MoneyGateGuard`, không có decorator tương đương, và AD-8 (`GRANT`) là thứ duy nhất ngăn nó ghi vào `users` — không phải ngăn nó chuyển coin. `MoneyGateGuard.canActivate` từ chối một handler đã đánh dấu trên context không phải HTTP (có test), nên hướng mặc định trong `apps/api` là đúng; nhưng đó là một guard của process này, không phải của process kia. Epic 3 hoặc Epic 4 phải quyết định: hoặc mọi hệ quả tiền đi qua `apps/api`, hoặc gateway cần cổng của riêng nó đọc cùng `canReceiveMoney`.

- source_spec: `_bmad-output/implementation-artifacts/spec-1-6-he-thiet-ke-cam-trai.md`
  summary: Bản sửa "focus chỉ chuyển khi dialog mở ra, không phải mỗi lần prompt đổi" đã vào code nhưng KHÔNG có test nào chạy qua nó.
  evidence: Vòng review 1 (P17) chỉ ra effect trong `session-expiry-provider.tsx` gọi focus mỗi khi `prompt` đổi nhận dạng, nên một 401 chạy nền tới trong lúc người dùng đã Tab sang một liên kết provider sẽ giật họ về. Bản sửa khoá vào chuyển tiếp đóng→mở và đúng. Nhưng không ca nào chứng minh: muốn dựng ca đó cần một lời gọi có xác thực THỨ HAI phát ra được từ trạng thái đang mở dialog, mà hôm nay không màn hình nào có — `/khai-ngay-sinh` ở trạng thái `signed-out` không render nút thử lại nào. Agent hiện thực đã viết một ca nháp bắn `fetch('/khong-ton-tai')` rồi tự xoá vì nó không đi qua seam và sẽ xanh bất kể code đúng hay sai; đó là quyết định đúng, và đây là phần dư của nó. Đóng được khi Epic 2 có một màn hình gọi API nhiều lần trong cùng một phiên.
